...
import { validateSummonerName, validateTagLine } from '../../utils/inputValidation';
...
const SearchBar = memo(({ 
  ...
  const handleSubmit = useCallback((e) => {
    ...
    if (!summonerInput.trim() || !summonerInput.includes('#')) {
      const errorMsg = placeholder || t('summoner.searchPlaceholder') + ' (예: 챌린저#KR1)';
    ...
    // 입력 검증
    const nameValidation = validateSummonerName(gameName);
    const tagValidation = validateTagLine(tagLine);
    ...
    if (!tagValidation.isValid) {
      setErrorMessage(tagValidation.error);
      return;
    }
    
    const currentLang = lang || i18n.language || 'ko';
    
    // variant가 hero인 경우 다른 URL 패턴 사용
    if (variant === 'hero') {
      const queryString = new URLSearchParams({ gameName, tagLine }).toString();
      navigate(`/summoner/${selectedRegion}?${queryString}`);
    } else {
      // 기존 패턴 유지
      const encodedSummonerName = encodeURIComponent(`${gameName}#${tagLine}`);
      navigate(`/${currentLang}/summoner/${selectedRegion}/${encodedSummonerName}`);
    }
  }, [summonerInput, selectedRegion, lang, i18n.language, navigate, t, placeholder, variant]);
  ...
  const inputClasses = useMemo(() => {
    const baseClasses = 'w-full focus:outline-none focus:ring-2 focus:ring-brand-mint bg-background-card dark:bg-dark-background-card';
    const errorClasses = errorMessage ? 'border-red-500' : 'border-border-light dark:border-dark-border-light';
    
    if (variant === 'hero') {
      return `${baseClasses} text-lg py-4 px-6 rounded-full border-2 focus:ring-offset-2 ${errorClasses}`;
    }
    
    return `${baseClasses} py-2 pl-4 pr-10 rounded-full border focus:ring-offset-2 ${errorClasses}`;
  }, [variant, errorMessage]);

  const buttonClasses = useMemo(() => {
    const baseClasses = 'absolute inset-y-0 right-0 flex items-center text-text-secondary hover:text-brand-mint focus:outline-none focus:ring-2 focus:ring-brand-mint focus:ring-offset-2 rounded-r-full';
    
    if (variant === 'hero') {
      return `${baseClasses} pr-6`;
    }
    
    return `${baseClasses} pr-3`;
  }, [variant]);

  const iconSize = variant === 'hero' ? 'w-6 h-6' : 'w-5 h-5';

  return (
    <form onSubmit={handleSubmit} className={`w-full ${className}`} role="search" aria-label="소환사 검색">
      <div className={variant === 'hero' ? 'w-full max-w-2xl mx-auto' : 'flex gap-2'}>
