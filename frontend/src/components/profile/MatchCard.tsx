...
const costColors: CostColors = { 1: '#808080', 2: '#1E823C', 3: '#156293', 4: '#87259E', 5: '#B89D29' };
const getCostBorderStyle = (cost: number): { border: string } => ({ border: `2px solid ${costColors[cost] || costColors[1]}` });
...
const getTraitHexColor = (style: number): string => {
    switch (style) {
...
// MatchCard 컴포넌트
const MatchCard: React.FC<MatchCardProps> = ({ match, userPuuid, isExpanded }) => {
  // ... (생략) ...
  const placement: number = match.placement || 0;
  const cardBorderColorClass: string = placement === 1 ? 'border-l-yellow-400' : placement <= 4 ? 'border-l-blue-400' : 'border-l-gray-500';
