...
const costColors: Record<number, string> = { 1:'#6B7280', 2:'#16A34A', 3:'#3B82F6', 4:'#9333EA', 5:'#FBBF24' };
const getCostColor = (c: number): string => costColors[c] || costColors[1];

const Unit: React.FC<UnitProps> = ({ unit, isCompact = false }) => {
  const costBorderColor = getCostColor(unit.cost);
  const isDarkMode = useDarkMode(); // 현재 다크 모드 여부 확인

  // 다크 모드일 때만 그림자 적용
  const textShadowStyle: string = isDarkMode ? '0 0 3px black, 0 0 3px black' : 'none';
