...
const logger = createComponentLogger('ProfileHeader');
...
interface ProfileHeaderProps {
  ...
  refreshCooldownMessage: string | null;
}

const ProfileHeader: React.FC<ProfileHeaderProps> = ({ account, region, onRefresh, isRefreshing, refreshCooldownMessage }) => {
      ...
      <div className="flex items-center gap-4">
        <img 
          ...
          onError={(e: React.SyntheticEvent<HTMLImageElement>) => { 
            logger.debug('프로필 아이콘 로드 실패', { profileIconId: account.profileIconId });
