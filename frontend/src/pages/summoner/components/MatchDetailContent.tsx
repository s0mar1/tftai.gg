...
const logger = createComponentLogger('MatchDetailContent');

const getPlacementColor = (p: number): string => (p === 1 ? '#F59E0B' : p <= 4 ? '#3B82F6' : '#6B7280');
...
const AIAnalysisView: React.FC<AIAnalysisViewProps> = ({ matchId, userPuuid }) => {
    ...
    useEffect(() => {
        const fetchAiAnalysis = async () => {
            ...
            try {
                ...
                logger.debug('AI 분석 응답 수신', { matchId });
                setAiAnalysis(response);
            } catch (err) {
                const error = err as ApiError;
                logger.error('AI 분석 실패', error as Error, { matchId, userPuuid });
                setAiError(error.response?.data?.error || error.message || 'AI 분석을 불러오는데 실패했습니다.');
            } finally {
                setAiLoading(false);
            }
        };
...
interface MatchDetail {
  info?: {
    ...
    accounts?: Record<string, { gameName: string }>;
  };
}
...
const MatchDetailContent: React.FC<MatchDetailContentProps> = ({ matchId, userPuuid, isCompact = false }) => { // isCompact prop을 받도록 추가
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        ...
        logger.error('매치 상세 정보 로드 실패', apiError as Error, { matchId });
        setError(apiError.response?.data?.error || apiError.message || '에러 발생'); 
      }
  ...
  const PlayerCard: React.FC<PlayerCardProps> = ({ participant }) => {
  ...
  return (
    <div>
      ...
      {tab === 'info' && (
        ...
        : (<div>{detail?.info?.participants.sort((a: Participant, b: Participant) => a.placement - b.placement).map((p: Participant) => <PlayerCard key={p.puuid} participant={p} />)}</div>)
      )}
