...
// UI 텍스트 번역 API
router.post('/ui', async (_req: Request<{}, {}, UITranslationBody>, _res: Response, _next: NextFunction) => {
  try {
    ...
    if (!supportedLanguages.includes(targetLanguage)) {
      return _res.status(400).json({
        success: false,
        _error: `지원하지 않는 언어입니다. 지원 언어: ${supportedLanguages.join(', ')}`
      });
    }
    ...
    logger.error('UI 번역 API 오류:', _error);
    return _res.status(500).json({
      success: false,
      _error: _error.message || '번역 중 오류가 발생했습니다.'
    });
  }
});

// 모든 언어 일괄 번역 API
router.post('/ui/all', async (_req: Request, _res: Response, _next: NextFunction) => {
  try {
    ...
    logger.error('일괄 번역 API 오류:', _error);
    return _res.status(500).json({
      success: false,
      _error: _error.message || '일괄 번역 중 오류가 발생했습니다.'
    });
  }
});
...
// 번역 캐시 상태 확인 API
router.get('/cache/status', async (_req: Request, _res: Response) => {
  try {
    ...
    logger.error('캐시 상태 확인 오류:', _error);
    return _res.status(500).json({
      success: false,
      _error: '캐시 상태 확인 중 오류가 발생했습니다.'
    });
  }
});

// 캐시 무효화 API
router.post('/cache/invalidate', async (_req: Request<{}, {}, CacheInvalidateBody>, _res: Response) => {
  try {
    ...
    logger.error('캐시 무효화 오류:', _error);
    return _res.status(500).json({
      success: false,
      _error: '캐시 무효화 중 오류가 발생했습니다.'
    });
  }
});
...
router.get('/status', async (_req: Request, _res: Response) => {
  try {
    ...
    logger.error('번역 상태 확인 오류:', _error);
    return _res.status(500).json({
      success: false,
      _error: '번역 상태 확인 중 오류가 발생했습니다.'
    });
  }
});
